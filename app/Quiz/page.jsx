...
export default function Page1() {
  ...
  const [correctAnswer, setCorrectAnswer] = useState("23rd August"); // Correct answer
  ...
  const handleAnswer = () => {
    ...
    if (selectedAnswer === correctAnswer) {
      setNote(
        <>
          <span className="text-green-600 font-bold">Correct!</span> This was
          done to commemorate the successful landing of Chandrayaan-3.
        </>
      );
    } else {
      setNote(
        <>
          <span className="text-red-600 font-bold">Incorrect.</span> The correct
          answer is <span className="font-bold">{correctAnswer}</span>. This was
          done to commemorate the successful landing of Chandrayaan-3.
        </>
      );
    }
      ...
      <div className="flex justify-center items-center min-h-[80vh] gap-3">
        ...
        <div className="grid grid-cols-1 p-4 bgQuiz h-[500px] w-[1000px] rounded-lg shadow-lg">
          <div className="flex justify-start items-center flex-col">
            <h2 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-slate-100 to-slate-300 mb-8">
              Quick Review
            </h2>
            <div className="flex justify-center flex-col gap-10 rounded-2xl bg-white shadow-xl p-8 w-full max-w-[600px]">
              <p className="text-xl font-semibold text-gray-800">
                When is National Space Day celebrated, and why?
              </p>
              <div className="grid grid-cols-2 grid-rows-2 gap-6 mt-2">
                {options.map((option, index) => (
                  <div
                    key={index}
                    className={`text-xl flex gap-2 items-center p-4 rounded-lg transition duration-300 ease-in-out cursor-pointer ${
              ...
              <div className="flex justify-between">
                <div className="flex flex-row gap-1">
                  Note:
                  {showResult && <p className="font-semibold">{note}</p>}
                </div>
                <button
